//
////EXERCISE 1
////============================================================================
//// Name        : live04-bags.cpp
//// Description : Playing with bags
////============================================================================
//
//#include <iostream>
//#include <set> // http://www.cplusplus.com/reference/set/set/
//#include <string>
//
//using namespace std;
//
//// enumeration type for actions on the bag
//enum action { add, del, qry, quit, none };
//
//// translation of strings into actions
//action str2action(string s) {
//	if (s == "add") return add;
//	if (s == "quit") return quit;
//	if (s == "del") return del;
//	if (s == "qry") return qry;
//	return none;
//}
//
//// a place in the bag (full declaration)
//struct item {
//	unsigned int val;
//	item* next;
//};
//
//#define BAG_AS_ARRAY_SIZE 1000000
//
//// We will implement a bag in three ways
//struct bag {
//	bool as_array[BAG_AS_ARRAY_SIZE];  // using arrays
//};
//// We do it for pedagogical purposes
//// mainly to emphasize the differences
//// We will try to keep the three bags aligned
//// but (for pedagogical purposes) the following code fails in doing so
//
//// Simple function to initialise the bag
//void initialise(bag& b) {
//
//	// Array
//	for (int i = 0; i < BAG_AS_ARRAY_SIZE; i++) {
//		b.as_array[i] = false;
//	}
//
//}
//
//// function to display the content of the bag
////void display_bag(bag b) {
////
////	//cout << "The bag is : " << endl;
////
////	// Array
////	// cout << " - (A) - : ";
////	for (int i = 0; i < BAG_AS_ARRAY_SIZE; i++) {
////		if (b.as_array[i])
////			cout << i << " ";
////	}
////	cout << endl;
////
////	return;
////
////}
//
//void insert(bag& b, unsigned int x) {
//
//	// Array
//	b.as_array[x] = true;
//}
//
//void deleter(bag& b, unsigned int x) {
//	
//	// Array
//	b.as_array[x] = false;
//}
//
//void querier(bag& b, unsigned int x) {
//
//	// Array
//	if (b.as_array[x] == true) {
//		cout << "T";
//	}
//
//	else {
//		cout << "F";
//	}
//	
//}
//
//// this function deals with actions on a bag
//void update(bag& b, action a, unsigned int x) {
//
//	switch (a) {
//	case add:
//		insert(b, x);
//		break;
//	case del:
//		deleter(b, x);
//		break;
//	case qry:
//		querier(b, x);
//		break;
//	case quit:
//		break;
//	case none:
//		break;
//	default:
//		break;
//	}
//
//	return;
//}
//
//int main() {
//
//	bag my_bag;
//	string my_act_str;
//	unsigned int x;
//
//	initialise(my_bag);
//
//	bool go_on = true;
//	while (go_on) {
//
//		// display_bag(my_bag);
//		cin >> my_act_str;
//		action act = str2action(my_act_str);
//		if (act == quit) {
//			go_on = false;
//		}
//		else {
//			cin >> x;
//			update(my_bag, act, x);
//		}
//
//	}
//
//	return 0;
//
//}


//EXERCISE 1
//============================================================================
// Name        : live04-bags.cpp
// Description : Playing with bags
//============================================================================



//------------------------------------------------------------------------------------------------------


//EXERCISE 2


//#include <iostream>
//#include <set> // http://www.cplusplus.com/reference/set/set/
//#include <string>
//
//using namespace std;
//
//// enumeration type for actions on the bag
//enum action { add, del, qry, quit, none };
//
//// translation of strings into actions
//action str2action(string s) {
//	if (s == "add") return add;
//	if (s == "quit") return quit;
//	if (s == "del") return del;
//	if (s == "qry") return qry;
//	return none;
//}
//
//// a place in the bag (full declaration)
//struct item {
//	unsigned int val;
//	item* next;
//};
//
//#define BAG_AS_ARRAY_SIZE 1000000
//
//// We will implement a bag in three ways
//struct bag {
//	int as_array[BAG_AS_ARRAY_SIZE];  // using arrays
//};
//// We do it for pedagogical purposes
//// mainly to emphasize the differences
//// We will try to keep the three bags aligned
//// but (for pedagogical purposes) the following code fails in doing so
//
//// Simple function to initialise the bag
//void initialise(bag& b) {
//
//	// Array
//	for (int i = 0; i < BAG_AS_ARRAY_SIZE; i++) {
//		b.as_array[i] = 0;
//	}
//
//}
//
// //function to display the content of the bag
//void display_bag(bag b) {
//
//	cout << "The bag is : " << endl;
//
//	//Array
//	cout << " - (A) - : ";
//	for (int i = 0; i < BAG_AS_ARRAY_SIZE; i++) {
//		if (b.as_array[i])
//			cout << i << " has " << b.as_array[i] << " amount of elements " << endl;
//	}
//	cout << endl;
//
//	return;
//
//}
//
//void insert(bag& b, unsigned int x) {
//
//	// Array
//	/*if (b.as_array[x] == true) {
//		item* next_item = new item[x+1];
//	}*/
//
//	/*else {
//	b.as_array[x] == true;
//	}*/
//
//
//
//	b.as_array[x] += 1;
//}
//
//void deleter(bag& b, unsigned int x) {
//	if (b.as_array[x] == 0) return;
//	// Array
//	b.as_array[x] -= 1;
//}
//
//void querier(bag& b, unsigned int x) {
//
//	// Array
//	if (b.as_array[x] > 0) {
//		cout << "T";
//	}
//
//	else {
//		cout << "F";
//	}
//
//}
//
//// this function deals with actions on a bag
//void update(bag& b, action a, unsigned int x) {
//
//	switch (a) {
//	case add:
//		insert(b, x);
//		break;
//	case del:
//		deleter(b, x);
//		break;
//	case qry:
//		querier(b, x);
//		break;
//	case quit:
//		break;
//	case none:
//		break;
//	default:
//		break;
//	}
//
//	return;
//}
//
//int main() {
//
//	bag my_bag;
//	string my_act_str;
//	unsigned int x;
//
//	initialise(my_bag);
//
//	bool go_on = true;
//	while (go_on) {
//
//		// display_bag(my_bag);
//		cin >> my_act_str;
//		action act = str2action(my_act_str);
//		if (act == quit) {
//			go_on = false;
//		}
//		else {
//			cin >> x;
//			update(my_bag, act, x);
//		}
//
//	}
//
//	return 0;
//
//}

//EXERCISE 3

//============================================================================
// Name        : live04-bags.cpp
// Description : Playing with bags
//============================================================================

#include <iostream>
#include <set> // http://www.cplusplus.com/reference/set/set/
#include <string>

using namespace std;

// enumeration type for actions on the bag
enum action { add, del, qry, quit, none };

// translation of strings into actions
action str2action(string s) {
	if (s == "add") return add;
	if (s == "del") return del;
	if (s == "qry") return qry;
	if (s == "quit") return quit;
	return none;
}

// a place in the bag (full declaration)
struct item {
	double val;
	item* next;
};

// We will implement a bag in three ways
struct bag {
	item* as_linked_items;             // using linked lists
};
// We do it for pedagogical purposes
// mainly to emphasize the differences
// We will try to keep the three bags aligned
// but (for pedagogical purposes) the following code fails in doing so

// Simple function to initialise the bag
void initialise(bag& b) {

	//Linked-list
	b.as_linked_items = nullptr;
}

// function to display the content of the bag
void display_bag(bag b) {

	cout << "The bag is : " << endl;

	// Linked List
	cout << " - (L) - : ";
	item* p = b.as_linked_items;
	while (p != nullptr) {
		cout << p->val << " ";
		p = p->next; // same as (*p).next
	}
	cout << endl;

	return;

}

void insert(bag& b, double x) {

	// Linked list
	// we allocate a new place for the new value
	item* i = new item;
	i->val = x;
	//(*i).val=x;

	// we make it point to the bag
	i->next = b.as_linked_items;
	// we set the bag now to start with the new place
	b.as_linked_items = i;
}

void deleter(bag& b, double x) {

	b.as_set.erase(x);

	double i;

	item* p = b.as_linked_items;
	item* p_previous = b.as_linked_items;

	while (p != nullptr) {
		p_previous = p;
		if (p->next == nullptr) {
			if (p_previous->val == x) {
				b.as_linked_items = nullptr;
				return;
			}
			return;
		}
		p = p->next; // same as (*p).next
		i = p->val;
		if (i == x) {
			p_previous->next = p->next;
			delete p;
			return;
		}
	}

}

void querier(bag& b, double x) {
	
	item* p = b.as_linked_items;
	double check;
	while (p != nullptr) {
		check = p->val;
		p = p->next; // same as (*p).next

		if (x == check) {
			cout << "T";
			return;
		}
		
	}

	cout << "F";
}

// this function deals with actions on a bag
void update(bag& b, action a, double x) {

	switch (a) {
	case add:
		insert(b, x);
		break;
	case del:
		deleter(b, x);
		break;
	case qry:
		querier(b, x);
		break;
	case quit:
		break;
	case none:
		break;
	default:
		break;
	}

	return;
}

int main() {

	bag my_bag;
	string my_act_str;
	double x;

	initialise(my_bag);

	bool go_on = true;
	while (go_on) {

		//display_bag(my_bag);
		//cout << "What's next? (actions = add x , quit)" << endl;
		cin >> my_act_str;
		action act = str2action(my_act_str);
		if (act == quit) {
			go_on = false;
		}
		else {
			cin >> x;
			update(my_bag, act, x);
		}

	}

	return 0;

}


